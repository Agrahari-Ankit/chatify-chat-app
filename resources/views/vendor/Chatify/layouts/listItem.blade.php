...
<table class="messenger-list-item contact-item" data-channel="{{ $channel->id }}">
    <tr data-action="0">
        ...
        <td>
        <p>
            {{ strlen($channel->display_name) > 12 ? trim(substr($channel->display_name,0,12)).'..' : $channel->display_name }}
...
@endif
@if($get == 'search_item')
<table class="messenger-list-item contact-item" data-channel="{{ $user->to_channel_id }}" data-messageid="{{ $user->id }}">
    <tr data-action="0">
        {{-- Avatar side --}}
        <td style="position: relative">
            <div class="avatar av-m"
                 style="background-image: url('{{ asset('storage/users-avatar/' . $user->user_avatar) }}');">
            </div>
        </td>
        {{-- center side --}}
        <td>
            <p>
                {{ strlen($user->display_name) > 12 ? trim(substr($user->display_name,0,12)).'..' : $user->display_name }}
                <span class="contact-item-time" data-time="{{$user->created_at}}">{{$user->created_at}}</span>
            </p>
            <span>
                {{-- Last Message user indicator --}}
                {!!
                    $user->from_id == Auth::user()->id
                    ? '<span class="lastMessageIndicator">You :</span>'
                    : '<span class="lastMessageIndicator">'. $user->user_name .' :</span>'
                !!}
                {{-- Last message body --}}
...
{{-- {{dd($user)}} --}}
    <table class="user-list-item" data-user="{{ $user->id }}">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p>{{ strlen($user->name) > 12 ? trim(substr($user->name,0,12)).'..' : $user->name }}   @if(Auth::user()->role=='QC')<a href="#" class="danger remove-user"><i class="fa fa-window-close"></i></a> @endif</p>
            </td>
        </tr>
    </table>
@endif
