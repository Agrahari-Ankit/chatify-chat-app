...
function messengerSearch(input) {
  ...
  if (!searchLoading && !noMoreDataSearch) {
    if (searchPage < 2) {
      ...
      $(".messenger-tab .search-recordss").html("");
    }
    ...
    $.ajax({
      ...
      success: (data) => {
        setSearchLoading(false);
        if (searchPage < 2) {
          $(".messenger-tab .search-recordss").html(data.records);
        } else {
          $(".messenger-tab .search-recordss").append(data.records);
        }
        // update data-action required with [responsive design]
        cssMediaQueries();
        // Pagination lock & messages page
...
let userIDD='';
  $("body").on("click", ".user-list-item .remove-user", function () {
    userIDD = $(this).closest(".user-list-item").attr("data-user");
    console.log('userIDD:', userIDD);
    app_modal({
      name: "remove-user",
    });
  });
function deleteUser(channel_id) {
  $.ajax({
    url: url + "/group-chat/delete-user",
    method: "POST",
    data: { _token: csrfToken, channel_id: channel_id, user_id: userIDD },
    dataType: "JSON",
    beforeSend: () => {
      // hide delete modal
      app_modal({
...
function adduserGroupChat(channel_id) {
  $.ajax({
    url: url + "/group-chat/adduser",
    method: "POST",
    data: { _token: csrfToken, channel_id: channel_id, user_id: auth_id },
    dataType: "JSON",
    beforeSend: () => {
...
function groupChatAddingModalInitnew(){
  const modalGroupChannel = $(".app-modal[data-name=add-group]")
  ...
  */
  ...
  $("body").on("click", ".search-records .user-list-item", function () {
    const userID = $(this).attr("data-user");
    const addedUserView = modalGroupChannel.find('.added-userss')
  ...
  /* -------- Remove User in group -------- */
  $("body").on("click", ".added-userss .user-list-item", function () {
    const userID = $(this).attr("data-user");
    console.log('hiii',userID)
    addedUserIds.splice(addedUserIds.indexOf(Number(userID)), 1)
    $(this).remove()
  ...
  /* -------- Create Group Channel -------- */
  $("#addGroupFormm").on("submit", (e) => {
    e.preventDefault();
    createGroupChat();
  });
  function createGroupChat() {
    const addGroupFormm = $("#addGroupFormm");
    // const groupNameVal = $.trim(addGroupForm.find('#group_name').val());
    const avatar = addGroupFormm.find('.upload-avatar').prop('files');
    const channeid = addGroupFormm.find('.channel_id').val();

    const formData = new FormData();
    formData.append("avatar", avatar ? avatar[0] : null);
    formData.append("channeid", currentChannelId());
    formData.append("user_ids", addedUserIds);
    formData.append("_token", csrfToken);

    $.ajax({
      url: addGroupFormm.attr("action"),
      method: "POST",
      data: formData,
      dataType: "JSON",
      processData: false,
      ...
      success: (data) => {
        if (data.error) {
          ...
          // reset form
          addGroupFormm.trigger("reset");
          addedUserIds.length = 0
          modalGroupChannel.find('.added-userss')?.html("")
          modalGroupChannel.find('.search-records')?.html("")
        }
      },
      error: () => {
...
$(document).ready(function () {
  ...
  $("body").on("click", ".messenger-list-item.contact-item", async function () {
    ...
    routerPush(document.title, `${url}/${channel_id}?messageid=${messageid}`);
    setCurrentChannelId(channel_id);
    updateSelectedContact(channel_id);
  ...
  $("body").on("click", ".messenger-infoView-btns .add-group", function () {
    app_modal({
      name: "add-group",
    });
  });
  ...
  $(".app-modal[data-name=remove-user]")
      .find(".app-modal-footer .delete")
      .on("click", function () {
        deleteUser(currentChannelId())
        app_modal({
          show: false,
          name: "remove-user",
        });
  ...
  // Delete group modal [on button click]
  $(".app-modal[data-name=add-group]")
      .find(".app-modal-footer .add-group")
      .on("click", function () {
        adduserGroupChat(currentChannelId())
        app_modal({
          show: false,
          name: "add-group",
        });
